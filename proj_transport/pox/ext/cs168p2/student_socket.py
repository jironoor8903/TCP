...
class StudentUSocket(StudentUSocketBase):
  ...
  def close(self):
    ...
    if self.state is CLOSED:
      ...
      ## Start of Stage 6 ##
      """set a pending FIN, and we provide code for you to do that—use self.fin_ctrl. The function you’ll use also receives a parameter to move to the next appropriate state when the FIN is actually sent, so be sure to fill that out."""
  ...
  def connect(self, ip, port):
    ...
    ## Start of Stage 1 ##
    ## start the procedure by sending the initial SYN packet
    ##  Build a new packet with the correct flags and sequence number and change the connection state
    # def new_packet(self, ack=True, data=None, syn=False):
  ...
  def tx(self, p, retxed=False):
    ...
    if (p.tcp.SYN or p.tcp.FIN or p.tcp.payload) and not retxed:
      ...
      ## Start of Stage 4 ##
      """ update the next sequence number to be sent in your send sequence space for the case that you send a packet with payload."""
      self.snd.nxt = (self.snd.nxt |PLUS| len(p.tcp.payload)) % (2**32)
      ## End of Stage 4 ##
    ...
    * Send pending ACKs and FINs
    ...
    ## End of Stage 1 ##
      ...
      if self.acceptable_seg(seg, payload):
        ## insert the packet into the rx_queue
        # pdb.set_trace()
    ...
    """If the packet with the smallest sequence number from the queue indicates that the packet is out
of order, simply set a pending ack and allow the rest of rx() to execute. """
    while not self.rx_queue.empty():
      seqno, packet = self.rx_queue.peek()
      if seqno |GT| self.rcv.nxt:
        self.set_pending_ack()
        break
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.srtt == 0:
      ...
      betaremainder = (1 - self.beta)
      alpharemander = (1 - self.alpha)
      self.rttvar = (betaremainder * self.rttvar) + self.beta * abs(self.srtt - rtt)
      self.srtt = (alpharemander * self.srtt) + (self.alpha * rtt)
  ...
  def handle_accepted_payload(self, payload):
    ...
    ## Start of Stage 2 ##
    """In handle_accepted_payload(), increment the next expected byte, update the receive window size
      accordingly, and add the payload to rx_data. Finally set a pending ACK.
  """
    # pdb.set_trace()
  ...
  def update_window(self, seg):
    ...
    ## Start of Stage 5 ##
    """"Correctly assign the send window in update_window() to the value the segment advertises. Keep in mind that we are not implementing congestion control."""
  ...
  def handle_accepted_ack(self, seg):
    ...
    """
    ## Start of Stage 4 ##
    """ update the variable that keeps track of unacknowledged sequence numbers so far."""
    self.snd.una = seg.ack
    ## End of Stage 4    ##


    ## Start of Stage 8 ##
    """remove packets from the retransmission queue when they are ACKed. Implement this in handle_accepted_ack(). Hint: RetxQueue has a helpful method"""
    acked_pkts = self.retx_queue.pop_upto(seg.ack)
    ## End of Stage 8 ##
  ...
  def handle_accepted_fin(self, seg):
    ...
    ## Start of Stage 6 ##
    """update the next sequence number expected in our receive sequence space"""
    self.rcv.nxt = seg.seq |PLUS| 1
    ## ACK the FIN.
    self.set_pending_ack()
    ##  we need to update our state, move to the appropriate state if the current state is ESTABLISHED
    ...
    if self.state == FIN_WAIT_1:
      if seg.ACK:
        if self.fin_ctrl.acks_our_fin(seg.ack):
          # If we received a FIN-ACK, transition to the TIME_WAIT state
  ...
  def check_ack(self, seg):
    ...
    if seg.ack |GE| snd.una and snd.nxt |GE| seg.ack:
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    ## Start of Stage 2 ##
    """
    TCP can only process the payload if the state is one of ESTABLISHED, FIN_WAIT_1, or FIN_WAIT_2,
  ...
  def maybe_send(self):
    ...
    ## Start of Stage 4 ##
    remaining = min(len(self.tx_data), self.snd.una |PLUS| self.snd.wnd |MINUS| self.snd.nxt)
    while remaining > 0:
      howmuchtosend = min(remaining, self.mss)
      payload = self.tx_data[:howmuchtosend]
      self.tx_data = self.tx_data[howmuchtosend:]
  ...
  def check_timer_retx(self):
    ...
    ## Start of Stage 8 ##
    """  Determine if the oldest packet in the queue needs to be retransmitted, i.e., it has been in the queue at least rto time. Retransmit it if so, retransmitting at most every time check_timer_retx() is called. Hint: RetxQueue has a get_earliest_pkt method."""
    """Get earliest pakcet in the queue"""
    if self.retx_queue.empty():
      return
    earliest_pkt = self.retx_queue.get_earliest_pkt()
    seq_no, packet = earliest_pkt
    time_in_queue = self.stack.now - packet.tx_ts
    if time_in_queue > self.rto:
        self.tx(packet, retxed=True)
      
        self.rto = min(self.rto * 2, self.MAX_RTO)
    ## End of Stage 8 ##m
      ## Start of Stage 9 ##
      # RTO should be doubled every time there is a retransmitted packet  RTO should never exceed MAX_RTO."""
      ## End of Stage 9 ##
